  ...
  apply(compiler) {
    ...
    compiler.plugin('compilation', compilation => {
      compilation.plugin('html-webpack-plugin-before-html-processing', (htmlPluginData, cb) => {
        ...
        const str = function (filesToInclude) {
          return `<script>var PreloadWebpackPlugin = {install: function(){var Head = document.getElementsByTagName('head').item(0);var Script= document.createElement('div');Script.innerHTML ='${filesToInclude}';Head.appendChild(Script);}}</script>`
        }


        if (htmlPluginData.html.indexOf('</head>') !== -1) {
          // If a valid closing </head> is found, update it to include preload/prefetch tags
          htmlPluginData.html = htmlPluginData.html.replace('</head>', str(filesToInclude) + '</head>');
        } else {
          // Otherwise assume at least a <body> is present and update it to include a new <head>
          htmlPluginData.html = htmlPluginData.html.replace('<body>', '<head>' + str(filesToInclude) + '</head><body>');
        }
        cb(null, htmlPluginData);
      });
    });
