...
export default {
  ...
  watch: {
    text: {
      handler (newVal) {
        const preRegexp = this.type === 'list' ? new RegExp('\\[', 'g') : this.hasBracket ? new RegExp('\\{' + this.pre + '\\[', 'g') : new RegExp(this.pre + '\\[', 'g')
        const afterRegexp = this.type === 'list' ? new RegExp(']', 'g') : this.hasBracket ? new RegExp(']}', 'g') : new RegExp(']', 'g')
