<template>
  ...
  <el-dialog title="选择商品" :visible="visible" class="g-wrapper" width="960px" :show-scroll-x="false" append-to-body
             height="498px" :response-limit=false :before-close="onClose">
      ...
      <el-col :span="24">
        <el-form ref="form" :model="model" label-width="60px" :inline="true">
          ...
          <el-form-item label="商品价格：">
            <el-form-grid>
              <el-form-grid width="60" style="margin-right: 0">
                <el-input type="" :min="0" v-model="model.minPrice" :max="model.maxPrice"
          ...
          <el-form-item label="商品分类：">
            <el-form-grid size="sm">
              <ns-droptree v-model="model.cate" placeholder=""
                           :url="$api.database.goods.queryGoodCategory"></ns-droptree>
            </el-form-grid>
          </el-form-item>
    ...
    <el-row class="tmp-choose__condition" :gutter="20">
      <el-col :span="16" class="condition-left">
        <el-table ref="table" :data="table.data" stripe
          ...
          <el-table-column property="outerId" label="商家编码" show-overflow-tooltip width="120">
            <template slot-scope="{row}">
              {{row.outerId ? row.outerId : '-'}}
            </template>
          </el-table-column>
  ...
  export default {
    ...
    props: {
      ...
      transSelectedData: {
        ...
        default (data) {
          let transData = {}
          transData[this.props.unique] = data.sysGoodsId
    ...
    methods: {
      ...
      onDelSelected: function (dataName, index, unique) {
        ...
        if (this.$refs.table) {
          for (var i = 0; i < this.table.data.length; i++) {
            if (unique === this.table.data[i].sysGoodsId) {
              // 删除表格选中
      ...
      onMinLimitInput: function () {
        this.limitInput('minPrice')
      },
      onMaxLimitInput: function () {
        this.limitInput('maxPrice')
      },
      // 价格限制输入
      limitInput (name) {
      ...
      toggleSelection: function (selected, rows) {
        for (var i = 0; i < rows.length; i++) {
          for (var j = 0; j < selected.length; j++) {
            if (rows[i].sysGoodsId === selected[j][this.props.unique]) {
      ...
      onSelectRow: function (selected, row) {
        ...
        for (var i = 0; i < selected.length; i++) {
          if (selected[i].sysGoodsId === row.sysGoodsId) {
            check = true
            break
          }
        }
      ...
      setSelectedData: function (check, row) {
        ...
        if (check) {
          ...
          for (var j = 0; j < showSelectedList.length; j++) {
            if (showSelectedList[j][this.props.unique] === row.sysGoodsId) {
