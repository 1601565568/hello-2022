...
export default {
  ...
  methods: {
    ...
    getEmployeeList () {
      ...
      let param = { pageNo: this.pagination4Emp.page, pageSize: this.pagination4Emp.size, auth: this.auth, switchAreaFlag: this.switchAreaFlag, areaId: this.departData.shopArea.value, status: this.departData.status, onlyOwner: this.onlyOwner }
      this.$http.fetch(this.guideUrl, param)
        .then(resp => {
          if (resp.result && resp.result.data && resp.result.data.length > 0) {
