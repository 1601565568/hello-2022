<template>
  <div>
    <page-table
      :title="title"
      :headerTip="headerTip"
      :class="[messageQA === 'QA' ? fuscousQA : fuscousIcon]"
    >
      <!-- 搜索 start -->
          ...
          <el-form-item label="参与门店：">
            ...
            >
              <template slot="btnIcon">
                <div class="self-btn">
                  {{
                    model.shopIdList && model.shopIdList.length
                      ? `已选择${model.shopIdList.length}个门店`
                      : "全部"
                  }}
                  <Icon type="shop" class="guideIds-icon"></Icon>
                </div>
      ...
      <template slot="table">
        <template>
          ...
          >
            ...
            <el-table-column prop="guideNames" width="90px" label="参与门店">
              <template slot-scope="scope">
                <span
                  class="scope-name_tip"
                  :class="[messageQA === 'QA' ? messageIconQA : messageIcon]"
                  slot="reference"
            ...
            <el-table-column prop="status" width="90px" label="状态">
              <template slot-scope="scope">
                <el-tag
                  :type="statusList[scope.row.state].color"
                  class="scope-name_tag"
                  >{{ statusList[scope.row.state].value }}</el-tag
                >
              </template>
            </el-table-column>
            ...
            <el-table-column prop="createName" width="120px" label="创建人">
            </el-table-column>
            <el-table-column prop="createTime" label="创建时间"> </el-table-column>
            <el-table-column width="100px" align="center" label="二维码">
              <template slot-scope="scope">
            ...
            <el-table-column align="center" width="100px" label="海报">
              <template slot-scope="scope">
                <div class="scope-name scope-name_num box-padding">
                  <Icon
                    type="ns-file-picture"
                    :class="[messageQA === 'QA' ? messageIconQA : messageIcon]"
                    @click="handlePreviewPoster(scope.row)"
                  />
                </div>
              </template>
            </el-table-column>
            <el-table-column prop="address" width="250px" label="操作">
              <template slot-scope="scope">
                <ns-button
                  type="text"
                  @click="handleDetail({ guid: scope.row.guid, id: scope.row.id })"
                  v-if="isShowEdit(scope.row)"
                  >编辑</ns-button
                >
                <ns-button
                  type="text"
                  @click="handleEnd(scope.row.guid, scope.row.createId)"
                  v-if="isShowEdit(scope.row)"
                  >结束活动</ns-button
                >
                <ns-button type="text" v-copy="scope.row.activityQrcode"
                  >复制链接</ns-button
                >
                <ns-button
                  type="text"
                  v-if="!(scope.row.state === 0 && type === 'Friends')"
                  @click="
                    handleAnalysis(
                      scope.row.guid,
                      scope.row.name,
    ...
    >
      ...
      <ShopList
        ...
        :type="type"
        @onNext="getOhter('next', handleShowDetail)"
