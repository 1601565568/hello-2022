<template>
  <div>
    <el-dialog title="选择优惠券" width="960px" :visible.sync="chooseCouponDialogVisible">
      <el-form
        ref="model"
        :inline="true"
        :model="model"
      ...
      <div class="warpper">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              {{
                scope.row.storeCouponType == 1
                  ? "代金券"
                  : scope.row.storeCouponType == 2
                  ? "折扣券"
                  : scope.row.storeCouponType == 3
                  ? "兑换券"
                  : "-"
              }}
            </template>
          ...
          >
            <template slot-scope="scope">
              ...
              >
              <!-- 折扣券 -->
              <span v-if="scope.row.storeCouponType == 2"
                >{{ getCouponValue(scope.row.storeCouponValue) }}折</span
              >
              <!-- 兑换券 -->
              <span v-if="scope.row.storeCouponType == 1"
                >{{ scope.row.storeCouponValue }}元</span
              >
            </template>
          </el-table-column>
          <el-table-column label="剩余数量" align="center">
            <template slot-scope="scope">
              <span v-if="scope.row.maxIssueAmount < 0">{{
                scope.row.maxIssueAmount
              }}</span>
              <span
                v-if="scope.row.channelConfigType == 1 && scope.row.maxIssueAmount >= 0"
                >{{ scope.row.limitAmount - scope.row.channelFreezeAmount }}</span
              >
              <span
                v-if="scope.row.channelConfigType == 0 && scope.row.maxIssueAmount > 0"
              >
                {{
                  scope.row.maxIssueAmount -
                  scope.row.hadIssueAmount -
                  scope.row.couponFreezeAmount
                }}</span
              >
              <span
                v-if="scope.row.channelConfigType == 0 && scope.row.maxIssueAmount == 0"
