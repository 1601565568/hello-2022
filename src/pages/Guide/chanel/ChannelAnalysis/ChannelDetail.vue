        ...
        >
          ...
          <el-table-column prop="addCount" label="添加人数" sortable="addCount">
            <template slot="header">
              添加人数
              <el-tooltip class="help" content="统计当前日期，通过所有渠道添加的好友总数（会进行以下场景排重仅计算为1：添加后删除又添加，同一个消费者一天添加多个员工）">
                <Icon type="ns-help"/>
              </el-tooltip>
            </template>
          </el-table-column>
          <el-table-column prop="deleteCount" label="删除人数" width="316" sortable="deleteCount">
            <template slot="header">
              删除人数
              <el-tooltip class="help" content="统计当前日期，员工主动删除的所有渠道的好友数（会进行以下场景排重仅计算为1：一天发生多次删除，多个员工删除同一个好友）">
                <Icon type="ns-help"/>
              </el-tooltip>
            </template>
          </el-table-column>
