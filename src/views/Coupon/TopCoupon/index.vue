                ...
                <el-radio-group v-model="coupon.condition" class="el-inline-block">
                  <el-radio  label="0" :value="0">不限
                    <el-form-item class="el-inline-block">
                      <el-tooltip placement="top" effect="light" stype=""><i class="el-icon-info"></i>
          ...
          <el-form-grid>
            <el-form-item prop = "role_id" >
              <template v-if="!expire">
                ...
                <el-tooltip content="请在授权时间到期之前进行优惠券的创建！" placement="right" effect="light"><i class="el-icon-info"></i></el-tooltip>
                <ns-button type="primary" round @click="onState()">设为失效</ns-button>
              </template>
              <template v-else>
                暂未授权或授权已过期，优惠券无法创建，请用 <a @click="grantAuth">主号登录授权</a>。<el-tooltip placement="top" effect="light"><i class="el-icon-info"></i>
