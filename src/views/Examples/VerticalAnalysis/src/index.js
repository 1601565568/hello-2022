      ...
      columns.forEach((column, index) => {
        ...
        if (!values.every(value => isNaN(value))) {
          sums[index] = values.reduce((prev, curr) => {
            ...
            if (!isNaN(value)) {
              Average[index] = ((prev + curr) / data.length).toFixed(2)
              return prev + curr
            } else {
          ...
          valueOther.forEach((columnValue, indexValue) => {
            var nanState = isNaN(parseInt(valueOther[indexValue]))
            if (nanState) {
              total += parseInt(valueOther[indexValue].split('￥')[1])
              totalState = true
            } else {
          ...
          if (totalState) {
            sums[index] = '￥' + total
            Average[index] = '￥' + (total / data.length).toFixed(2)
          } else {
            sums[index] = '-'
            Average[index] = (total / data.length).toFixed(2) + '%'
          }
