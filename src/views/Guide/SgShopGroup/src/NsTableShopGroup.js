...
export default {
  ...
  data: function () {
    ...
    const tableButtons = [
      {
        'func': function () {
          this.$emit('batchClearMember')
        },
        'name': '批量清理非员工微信'
      },
      {
        'func': function () {
          this.$emit('batchSetRobotRule')
        },
        'name': '批量设置AI通知'
      },
      {
        'func': function () {
          this.$emit('groupCheck')
        },
        'name': '门店群排重查漏'
      },
      {
    ...
    return {
      ...
      operatingStatus: [{ label: '全部', value: 0 }, { label: '已登录', value: 1 }, { label: '未登录', value: 2 }],
