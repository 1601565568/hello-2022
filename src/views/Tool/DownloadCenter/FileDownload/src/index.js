  ...
  methods: {
    ...
    onSubmitAndDownload: function () {
      that.$refs.permissionForm.validate((valid) => {
        if (valid) {
          that.submit({
            api: that.$api.core.exportTask.validateCaptcha,
            params: {
              mobile: that.model.mobile,
    ...
    onBeforeDownload: function (title, data) {
      if (!(data.export_status === 2)) {
        that.$notify.error('生成状态为“生成完成”才能下载')
        return false
      }
      if (data.export_status === 2) {
        that.downloadData.id = data.id
        that.downloadData.downLoad_addr = data.downLoad_addr
        that.downloadData.aliasFileName = data.alias_file_name
    ...
    onDownload: function () {
      that.$notify.info('下载中... (下载完成时间预计1-4分钟，请耐心等待)')
      this.$http.fetch(this.$api.core.exportTask.download, {id: that.downloadData.downLoad_addr, exportId: that.downloadData.id})
        .then((resp) => {
          ...
          link.href = url
          var fileName = that.downloadData.aliasFileName
