    ...
    <template slot="table">
      ...
      <el-table ref="table" :data="_data._table.data" class="template-table__main"
        ...
        <el-table-column :show-overflow-tooltip="true" type="default" prop="create_time"
                         label="创建时间" dbcolumn="create_time" column="create_time" align="center"
                         :sortable="false" :width="200">
        </el-table-column>

        <el-table-column :show-overflow-tooltip="true" type="default" prop="trade_tag_name"
                         label="标签名称" dbcolumn="trade_tag_name" column="trade_tag_name" align="left"
                         :sortable="false">
        </el-table-column>

        <el-table-column :show-overflow-tooltip="true" type="default" label="标签类型"
                         align="center" :sortable="false" width="100">
          <template slot-scope="scope">
            <template v-if="scope.row.trade_tag_type == 0">其他</template>
            <template v-else-if="scope.row.trade_tag_type == 1">预售</template>
            <template v-else="scope.row.trade_tag_type == 2">赠品</template>
          </template>
        </el-table-column>
        ...
        <el-table-column :show-overflow-tooltip="true" type="default" label="标签类型"
          ...
          <template slot-scope="scope">
            <template v-if="scope.row.trade_tag_type == 0 && scope.row.trade_tag_content != null">{{scope.row.trade_tag_content}}</template>
