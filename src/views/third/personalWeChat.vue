...
<div class='person-page'>
  <div class='card flex flex-between'>
    <div>
      <div class='fzi28'>南讯软件</div>
      <div class='fzi36' style='margin-top:0.1rem'>{{query.name}}</div>
      <div class='fzi28'>{{query.shopName}}  {{query.job==1?'店长':query.job==0?'导购':''}}</div>
      <div class='fzi28' style='margin-top:0.1rem'>{{query.mobile}}</div>
    </div>
...
export default {
  ...
  mounted: function () {
    ...
    this.$route.query.shopName = decodeURIComponent(this.$route.query.shopName)
